...
web setup wizard. On macOS, the default “remember” option stores it in the system Keychain under
service `eu.vantora.pylux-web`; otherwise it remains in process memory only. It is cleared from the
form after validation and is never written to browser storage or committed.
...
- The bridge never sends the NPSSO token back to the browser or writes it to a plaintext file.
